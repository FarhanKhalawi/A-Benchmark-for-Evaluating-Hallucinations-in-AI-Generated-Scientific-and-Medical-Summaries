...
CSV_NAME = "results_llm_judge100s.csv"
...
def ensure_required_columns(df: pd.DataFrame, model_name: str) -> pd.DataFrame:
    ...
    if "hallucination_score" not in df.columns:
        raise ValueError(
            f"Column 'hallucination_score' is missing for {model_name}"
        )

    df["hallucination_score"] = pd.to_numeric(
        df["hallucination_score"],
        errors="coerce"
    )

    df = df.dropna(subset=["hallucination_score"])

    # Keep valid score range only
    df = df[
        (df["hallucination_score"] >= 0.0)
        & (df["hallucination_score"] <= 1.0)
    ]
...
if not model_data:
    raise RuntimeError("No data loaded. Run LLM-as-a-Judge evaluation first.")
...
# =============================================================
# FIGURE 1 — LLM-as-a-Judge hallucination score boxplot
# =============================================================
...
ax0.set_ylabel(
    "LLM-as-a-Judge hallucination score",
    **LABEL_KW
)
...
out_path1 = "Figure_LLMJudge_hallucination_boxplot100s.png"
...
# =============================================================
# FIGURE 2 — ECDF of LLM-as-a-Judge hallucination scores
# =============================================================
...
out_path2 = "Figure_LLMJudge_hallucination_ecdf100s.png"
