"""
FactScore — Hallucination Detection (NO VALIDATION - processes all summaries)
================================================================
Two-step pipeline:
...
ALL summaries are processed regardless of format.
...
Output:
  outputs(Model)_pubmed_abstract/results_with_factscore1000s.csv
"""
...
def is_valid_summary(summary: str) -> dict:
    """No validation - all summaries are processed"""
...
def _process_one_sample(idx: int, article: str, summary: str, n: int) -> dict:
    ...
    # ── Validation check (only skips ERROR summaries) ────────
...
for CURRENT_MODEL in active_models:
    ...
    print(f"  VALIDATION       : DISABLED (all summaries processed)")
    print(f"{'#'*72}")
    ...
    print(f"\n  Validity check: {pre_valid}/{n} summaries will be processed (validation disabled).")
